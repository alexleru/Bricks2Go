...
class DetailOfModelAdapter : RecyclerView.Adapter<DetailOfModelAdapter.VHDetails>(),
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): VHDetails = VHDetails(
        if (viewType == R.layout.btn_of_list_detail)
            LayoutInflater.from(parent.context).inflate(
                R.layout.btn_of_list_detail
                , parent, false
            ) else LayoutInflater.from(parent.context)
            .inflate(R.layout.item_of_list_detail, parent, false)
    )

    override fun getItemCount(): Int = 30 + 1
    ...
    override fun getItemViewType(position: Int): Int {
        return if (position == 30) R.layout.btn_of_list_detail
